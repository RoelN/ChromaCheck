...
(function(){
  ...
  // this is to check that the browser supports CSS.supports("font-tech()") by checking for a format all modern browsers universally support (woff). All browsers that support font-format() also support font-tech()  
  var fontTechSupported = CSS.supports("font-format(woff)");

  function checkSupportUsingCSSSupports() {
      var res = {};
      res.cbdt = CSS.supports("font-tech(color-cbdt)");
      res.colr = CSS.supports("font-tech(color-colrv0)");
      res.sbix = CSS.supports("font-tech(color-sbix)");
      res.svg = CSS.supports("font-tech(color-svg)");
      res.colrv1 = CSS.supports("font-tech(color-colrv1)"); 
  
      // Add class to HTML tag for each supported color format
      for (var key in res) {
        if (res.hasOwnProperty(key)) {
          res[key] && root.classList.add('chromacheck-'+key);
        }
      }
  }

  if (fontTechSupported) {
    checkSupportUsingCSSSupports();
  }
  else {
    var cls = 'chromacheck-',
    runs = 20,
